...
| `discord.test.sh` | Rich Presence: handshake, rotating pages, live "Model · Effort · N sessions" state line (main-thread model beats a newer sidechain/advisor line; `discordShowModel:false` drops it), https image URL passes verbatim, a rejected activity's error clears on the next accepted one (no reconnect), clear-on-disable, shipped default app ID, Discord-not-running degradation |
