...
- `test/migration.test.sh` is the only suite WITHOUT a `*_HOME` pin (fake
  ...
  backwards-only fills, reduced motion. `test/tray.test.sh` starts 3 more
  servers on its own port 4887 with a fake tray (`PULSE_TRAY_TEST_SCRIPT`,
  ~30 s) and parses the generated tray.ps1 with pwsh when `PWSH=/path/to/pwsh`
  is set or pwsh is on PATH (else SKIP). `test/strip-heal.test.sh` (mock GitHub
...
- Manual Windows RC pass, beyond the list above: the real taskbar strip
  ...
  mini view with cwd = home. rc.3 OpenUsage removal: a start with
