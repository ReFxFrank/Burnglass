...
3. **`~/.burnglass` is the ONLY writable location** (config, logs, history,
   ...
   `metersCachePath()` null) and skips the strip refresh.
...
- Colour only from tokens (no hex in JSX/section CSS); source colours
  ...
  hover/focus (lite + reduced motion drop them). 360–3440 px with no
  horizontal scroll.
- Width: `--content-max` 2000px (INCLUDING the 24 px gutters each side);
  past it `.content` is centred in `.main` (`margin-inline: auto`) and the
  `.topbar` padding is `max(var(--gutter), calc((100% - var(--content-max))
  / 2 + var(--gutter)))` so its contents sit on the content edges at every
  width; the rail stays pinned left. The rail is 8 px side padding + 8 px
...
for the strip: `running` / `kills.log` / `launches.log` / `sticky`). Build-time
...
- `test/migration.test.sh` is the only suite WITHOUT a `*_HOME` pin (fake
  ...
  checks and FAILs the payload check. `test/strip-heal.test.sh` (mock GitHub
  release by tag, fake strip exes, `PULSE_STRIP_PROC_STUB`) owns ports
  5861–5885 and fails fast if one is busy; `STRIP_HEAL_SERVER=<server.js>`
  ...
  `test/web-notices.test.sh` part 2 also checks wide-screen centring and
  top-bar alignment (1440–3440) and the rail source-row fit/ellipsis.
