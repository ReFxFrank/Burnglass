...
- The built-in animated Clawd links (`DISCORD_DEFAULT_ART_BASE` = the
  `v2.0.0` tag's `.github/assets/discord/` via raw.githubusercontent.com): every
  2.0.1+ presence shows them, so the tag and those four `-512.gif` files at it
  must stay forever.
...
- `.github/workflows/release.yml` — 3-OS matrix (win/linux/mac arm64) via
  ...
  tags).
- `README.md` = the product page (pitch, hero, feature tour with a picture
  per feature, quick start, short upgrade notes, docs index). Long REFERENCE
  material lives in `docs/` (`README.md` index + `configuration`,
  `limits-and-budgets`, `discord`, `windows`, `custom-sources`,
  `how-it-works`, `privacy-security`, `api`, `upgrading-from-pulse`,
  `development`); each page opens with a one-line intro and a back link to
  the README. A change to a config key, flag, route, default or file path
  must update the matching docs page (and the README only if the headline
  claim changes). Docs reference images as `../.github/assets/…`.
- `.github/assets/` — README / docs art, ALL from fixture data (sample
  numbers, `/home/you` or `C:\Users\you` paths, no update banner, no real
  reach counts): `logo.svg` / `logo-light.svg` (the Glass lockup, dark /
  light via `<picture>`); dashboard shots at 1440 dark — `hero.png` +
  `hero-light.png` (the `<picture>` pair), `limits.png`, `spend.png`,
  `breakdown.png`, `activity.png`, `meshy.png`, `system.png` (a Windows
  install look, page data rewritten in the browser), `wide.png` (2560 wide,
  scaled to 1600), `mini.png` and `mobile.png` (dark + light side by side);
  `strip-popover.png` (2×, show at `width="372"`), `strip-open.gif` (open
  animation; its taskbar strip is an HTML mock of `StripForm.Render`'s usage
  view — provider glyph + % USED, second line amber at the first threshold,
  no labels, no price — keep it in step with the real strip), `tray.png` (the four tray states), `statusline.png` (2×,
  `width="633"`) and `summary.png` (2×, `width="498"`); and `discord/` —
  the Clawd Rich Presence GIFs: 160 px `working`/`thinking`/`waiting`/`idle`
  `.gif` (shown inline), their `-512.gif` originals (for users to HOST;
  linked, never inlined — two are > 400 KB), `clawd-pixel.gif`, the
  clawd-tank MIT `LICENSE-clawd-tank.txt` and a `README.md` (source commit,
  state → slot → config key, fan-art note). **Size rule:** every image shown
  inline must stay < 400 KB (PNG at device scale 1, or 2× with an explicit
  `width` attribute); GIFs as small as practical; every `<img>` gets alt text.
...
- Anthropic prices (verified 2026-09-24): Opus 5.5 $4/$20 with 0.05× cache
  ...
  $5/$25 (+ fast $10/$50; with Opus 5.5's $8/$40 the only fast-mode models); Sonnet 5 $2/$10
...
- Brand ("Glass", v2.0.0): the brand kit's usage rules are summarised in the
  ...
  lockup) + `logo-light.svg` via `<picture>` (full inventory under Layout).
