...
3. **`~/.burnglass` is the ONLY writable location** (config, logs, history,
   ...
   Nothing under `~/.pulse` is ever moved, renamed or deleted.
...
4. **Binds 127.0.0.1** by default; all state-changing endpoints require
   ...
   GET /api routes have a DNS-rebinding guard (`allowRead`). The wire header
...
6. **Network calls, exhaustively:** GitHub version check + community-reach
   ...
   `UPDATE_REPO = 'ReFxFrank/Burnglass'`), api.anthropic.com + chatgpt.com
   meters (opt-in), api.meshy.ai balance + task history (opt-in, Bearer key
   from config), Discord local socket (opt-in, not network). Usage data never
...
- `sections/<Name>.jsx` + `<Name>.css` (CSS scoped under `.sec-<name>`), in
  ...
  hooks, `Integrations` toggle grid, `DiscordImagesForm`, `LogTail`).
...
- Colour only from tokens (no hex in JSX/section CSS); source colours
  ...
  hover/focus (lite + reduced motion drop them). 360–1920 px with no
  horizontal scroll.
...
| Home + ~/.pulse migration (v2.0.0) | `envv(name)` (`BURNGLASS_<name>` wins, `PULSE_<name>` permanent alias; empty BURNGLASS value = unset) is used for EVERY env read. `appHome()`: `BURNGLASS_HOME`/`PULSE_HOME` verbatim (never migrated) > `~/.burnglass` if it exists > `~/.pulse` if it exists (pre-migration, NOT memoized) > `~/.burnglass` (fresh; writers mkdir lazily). `migrateHome()` runs FIRST in the listen callback — only once THIS process owns its port (a v2 losing the bind to a running v1 publishes nothing) and before anything reads config; short-lived commands (`--statusline`, `--summary`, `--mode-hook`, setup printers, `--startup`, `--stop`…) only resolve, never migrate. Copy = allowlist `MIGRATE_FILES` (config, meshy, modes.jsonl, discord-presence, strip*.json) + `history/YYYY-MM.json` (skips `*.tmp`); skipped names (server.json, tray.ps1, logs, strip-web/, webview-strip/, bin/…) are listed in the marker. Stage `~/.burnglass.migrating-<pid>-<6hex>` gets the `STAGE_SENTINEL` first; `renameWithRetry` publishes it atomically (EPERM/EACCES/EBUSY ×8 @125 ms, never once the target exists); marker `migrated-from-pulse.json` `{from, at, by, copied, skipped}`; a racing peer's win is adopted; any error → degraded run on `~/.pulse` (`homeMigration.status:'failed'`), retried next start; `sweepStaleMigrationStages` removes only exact-pattern dirs holding a sentinel older than 1 h. `adoptHome(dir)` resets every per-home cache. Legacy compat (only when `legacyCompatHome()` — not pinned, exists, not itself the home): `server.json` mirror + freshest-live `readRuntimeFile`, `refreshLegacyTrayScript`, `readModes` merges `~/.pulse/modes.jsonl`, `readHistory` merges `legacyHistoryDir()` month files READ-ONLY per cell via `pickCell` (new home wins ties), `scrubLegacyMeshyKey` after the copy and on every v2 key change/clear. Payload: `brand`, `home`, `exeName` (null from source), `homeMigration` (`{status:'migrated'\|'failed', from, at, by, justNow?, error?}` or null). UI: App `HomeNotices` (dismiss stored in `pulse-home-notice` = the marker's `at`, shown ≤14 days) + System "Data folder" fact. Suite: `test/migration.test.sh` (fake `$HOME`, NO home env) |
| Claude Code integrations check (read-only) | `claudeIntegrations()` (mtime-memoized read of `claudeDir()/settings.json`): `statusLine.command` + `hooks.*[].hooks[].command` carrying `--statusline` / `--mode-hook` → `commandTarget` (quoted or first token, skipping `env` and leading `NAME=value` assignments — a value there may be a secret; `node <script>` → the script) → `{kind, event, target, exists (null when not an absolute path), legacyName}`. `payload.integrations` strips the raw `command`; `warnBrokenIntegrations` logs once per distinct broken set at start; `printExistingIntegration` adds a "replace this" hint to `--statusline-setup` / `--effort-setup`. NEVER writes `~/.claude`. UI: App `HomeNotices` warn bar for `exists === false`, System "Claude Code" fact (status line / effort hook → exe name, "old name, still works") |
| Windows daemon | `--daemon-child`, `windowsHide`, `<home>/burnglass.log` (`openLogFile` is DEFERRED until after the migration so nothing new lands in `~/.pulse`; earlier lines are back-filled), `--stop`, `--install-shortcuts` (`Burnglass` / `Burnglass - Stop`); the same-or-newer-already-running branch honours `--no-open` (a sign-in copy never pops a browser) |
...
`OpenUsageTray.exe` alongside the server; Burnglass only STARTS it, never
...
that is missing does NOT fall back), `strip` (`=== true` — launch Burnglass's
own `burnglass-strip.exe` companion (`pulse-strip.exe` still found)) + `stripPath` (exe override; same
...
`PULSE_NO_TRAY_SPAWN` (suppress the tray launch), `PULSE_MESHY_API` /
...
choice, so a Linux suite can act as `pulse.exe`/`burnglass.exe`). Build-time
...
5. Wait ~3–4 min, then verify via the release-by-tag API: all NINE assets —
   ...
   installer + their aliases) are built only on the windows-latest job — if
   that job fails, the other OS binaries still publish, so check the asset
   COUNT, not just that a release appeared.
...
- `test/migration.test.sh` is the only suite WITHOUT a `*_HOME` pin (fake
  ...
  installer, the make-exe icon stamping) cannot run here — list it for the
  manual Windows RC pass instead of claiming it tested.
